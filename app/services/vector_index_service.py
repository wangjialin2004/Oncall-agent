...
class VectorIndexService:
    ...
    def index_single_file(self, file_path: str):
        ...
        try:
            # 1. 抽取文件内容，统一转换为 Markdown 风格文本
            extracted = document_extraction_service.extract_file(str(path))
            content = extracted.content
            logger.info(
                f"抽取文件: {path}, 内容长度: {len(content)} 字符, "
                f"图片={extracted.metadata.get('image_count', 0)}, "
                f"表格={extracted.metadata.get('table_count', 0)}"
            )
    ...
    def rebuild_collection_and_reindex(
        self,
        directory_path: str | None = None,
        confirm: bool = False,
    ) -> IndexingResult:
        """显式重建 Milvus collection 并重新索引目录。"""
