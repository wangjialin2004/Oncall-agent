...
class HarnessContext:
    ...
    active_attachment_index: str = ""
    active_attachments: tuple[AttachmentReference, ...] = field(default_factory=tuple)
...
class ContextBuilder:
    ...
    def build(
        ...
        *,
        ...
        try:
            if owner_key and session_id and self.history_max_turns > 0:
                all_turns = conversation_service.get_turns(owner_key, session_id) or []
        except Exception:
            all_turns = []
        active_attachments = self._collect_active_attachments(all_turns)
        ...
        return HarnessContext(
            system_prompt=self._build_system_prompt(
                ...
                active_attachment_index=active_index,
            ),
            history_messages=history_messages,
            active_attachment_index=active_index,
            active_attachments=tuple(active_attachments),
        )
    ...
    def _format_turns_for_summary(self, turns: Sequence[dict[str, Any]]) -> str:
        ...
        for turn in turns:
            ...
            user_message = self._history_user_text(turn)
            ...
            refs_hint = attachment_reference_service.format_turn_refs_for_summary(turn)
            if refs_hint:
                lines.append(f"第 {index} 轮 {refs_hint}")
        return "\n".join(lines)

    @staticmethod
    def _collect_active_attachments(
        turns: Sequence[dict[str, Any]],
    ) -> list[AttachmentReference]:
        """Deduplicate ``attachment_refs`` across turns, recency-first.

        Same file appearing in multiple turns keeps the most recent
        (i.e. last appended) reference, since later references may
        carry richer keyword/summary data after re-indexing.
        """
        ...
        for turn in turns:
            for raw_ref in turn.get("attachment_refs") or []:
                ...
                if reference is None:
                    continue
                if reference.file_id not in deduped:
    ...
    def _build_system_prompt(
        ...
        *,
        ...
        if active_attachment_index:
            sections.append(
                "本会话已上传附件的索引（file_id 稳定持久化，可在 system/工具结果中按 "
                "file_id 出现；如需引用某文件的全文细节，请在回答中点名 file_id，"
                "上层会自动按 file_id 重新加载原始内容注入到当前对话）：\n"
                f"{active_attachment_index}"
            )
    ...
    def _compact_turn(self, turn: dict[str, Any]) -> dict[str, Any]:
        ...
        compact["user_context"] = self._compact_history_message(
            str(turn.get("user_context") or "").strip()
        )
    ...
    def _history_user_text(self, turn: dict[str, Any]) -> str:
        user_message = str(turn.get("user_message") or "").strip()
        user_context = str(turn.get("user_context") or "").strip()
        if not user_context:
            return self._compact_history_message(user_message)
        combined = f"{user_context}\n\n用户问题：\n{user_message}" if user_message else user_context
        return self._compact_history_message(combined)
